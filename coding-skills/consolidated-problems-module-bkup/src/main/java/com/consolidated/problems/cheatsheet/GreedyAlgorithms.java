...
public class GreedyAlgorithms {
	 ...
	 */
	public int maxActivities(int[] start,
			int[] end) {
		...
		for (int i = 0; i < n; i++)
			intervals[i] = new Interval(start[i],
					end[i]);
	...
	public void findMeetingsInOneRoom(int[] start,
			int[] end) {
		if (start.length == 0 || end.length == 0)
			return;
		int n = start.length;
	...
	public boolean canAttendAllMeetings(
		...
		Arrays.sort(intervals, (ob1,
				ob2) -> ob1.start - ob2.start);

		for (int i = 0; i < intervals.length
				- 1; i++) {
			if (intervals[i].end > intervals[i
					+ 1].start)
				return false;
		}
		return true;
	...
	public int minMeetingRooms1(
		...
		int activeMeetings = 0,
				numMeetingRooms = 0, i = 0, j = 0;
		while (i < len && j < len) {
			if (startTime[i] < endTime[j]) {
				activeMeetings++;
				numMeetingRooms = Math.max(
						numMeetingRooms,
						activeMeetings);
				i++;
			} else {
	...
	public int minMeetingRooms3(
		...
		for (Interval i : intervals) {
			timesMap.put(i.start,
					timesMap.getOrDefault(i.start,
							0) + 1);
			timesMap.put(i.end, timesMap
					.getOrDefault(i.end, 0) - 1);
		}
	...
	public int minPlatformRequired1(int[][] arr,
			int[][] dep) {
	...
	// Approach2: Using TreeMap
	public static int minPlatformRequired2(
			int[] arrv, int[] dep) {
		...
		for (int i = 0; i < arrv.length; i++) {
			map.put(arrv[i],
					map.getOrDefault(arrv[i], 0)
							+ 1);
			map.put(dep[i],
					map.getOrDefault(dep[i], 0)
							- 1);
		}
		...
		for (int key : map.keySet()) {
			count += map.get(key);
			noOfPlatform = Math.max(noOfPlatform,
					count);
		}
		return noOfPlatform;
	}
		...
		for (Interval interval : intervals) {
			if (interval.end < newInterval.start) { // Right New Interval(No Overlap)->Eg:
				...
				newInterval = new Interval(
						Math.min(interval.start,
								newInterval.start),
						Math.max(interval.end,
								newInterval.end));
			}
		}
		result.add(newInterval);
	...
	public List<Interval> insert(
			...
			Interval newInterval) {
		int i = 0;
		while (i < intervals.size() && intervals
				.get(i).end < newInterval.start)
			i++;
		while (i < intervals.size() && intervals
				.get(i).start <= newInterval.end) {
			newInterval = new Interval(
					Math.min(
							intervals
									.get(i).start,
							newInterval.start),
					Math.max(intervals.get(i).end,
							newInterval.end));
			intervals.remove(i);
		}
	...
	public List<Interval> insert3(
		...
		int startingIndex = binarySearch(
				intervals, newInterval);
		// Add all the intervals before the startinIndex in the result list
		result.addAll(intervals.subList(0,
				startingIndex));

		for (int i = startingIndex; i < intervals
				.size(); i++) {
	...
	public List<Interval> merge(
		...
		for (int i = 1; i < intervals
				.size(); i++) {
			Interval currInterval = intervals
					.get(i);
			if (prevInterval.end >= currInterval.start) {
				prevInterval.end = Math.max(
						currInterval.end,
						prevInterval.end);
			} else {
	 ...
	 */
	private final int[] coins = { 1, 2, 5, 10, 20,
			50, 100, 200, 500, 2000 };
	...
	public static int maximizeToys(int[] prices,
			int k) {
		if (k < 1) return 0;
		Arrays.sort(prices);
		int count = 0;
	...
	public int maximumSwap2(int num) {
		char[] chars = Integer.toString(num)
				.toCharArray();
		int maxIndex = chars.length - 1, x = 0,
				y = 0;
	...
	public int[] maxNumber(int[] nums1,
			int[] nums2, int k) {
		int get_from_nums1 = Math
				.min(nums1.length, k);
		int[] maxNumber = new int[k];
		for (int i = Math.max(k - nums2.length,
				0); i <= get_from_nums1; i++) {
			int[] res1 = new int[i];
	...
	public boolean compare(int[] nums1,
			int start1, int[] nums2, int start2) {
		for (; start1 < nums1.length
				&& start2 < nums2.length; start1++, start2++) {
	...
	public String removeDuplicateLetters1(
			String s) {
		int[] hash = new int[26];
	 ...
	 */
	// Approach1: Assign the elements in forward
	// & reverse direction
	...
	// Approach2: Assign the elements first, after
	// that reverse the elements when char is 'D'.
	...
	public void huffmanCoding(String str) {
		Map<Character, Integer> charFreqMap = findCharFreq(
				str);

		// 2.Create a Priority Queue, which is used to build min heap based on char frequency
		PriorityQueue<HuffmanCode> queue = new PriorityQueue<>(
				charFreqMap.size(),
				new Comparator<HuffmanCode>() {
					public int compare(
							HuffmanCode o1,
							HuffmanCode o2) {
						return o1.count
		...
		for (Entry<Character, Integer> entry : charFreqMap
				.entrySet()) {
	 ...
	 */
	public void encodeString(HuffmanCode root,
			String binaryStr,
			Map<Character, String> encodedData) {
		if (root.left == null
				&& root.right == null
		...
		encodeString(root.left, binaryStr + "0",
				encodedData);
		encodeString(root.right, binaryStr + "1",
				encodedData);
	}
