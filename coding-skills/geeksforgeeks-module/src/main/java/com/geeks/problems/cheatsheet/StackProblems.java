...
public class StackProblems {
	...
	public void evaluatePosfixExpression(
			String expr) {
	...
	public int calculator1(String s) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (Character.isDigit(ch)) {
				...
				while (i + 1 < n
						&& Character.isDigit(s
								.charAt(i + 1))) { // Collect all the digits from the input
					digit = digit * 10
							+ s.charAt(i + 1)
							- '0';
					i++;
				...
				result = result * stack.pop()
						+ stack.pop(); // 1st pops the sign, 2nd pops the prev calculation
			}
		}
		return result;
	}
	...
	private int arithmeticOperation(char ch,
			int b, int a) {
		int result = 0;
		switch (ch) {
			case '+':
	...
	public static void nextGreaterElement2(
			int[] a) {
		Stack<Integer> stack = new Stack<>();
		stack.push(a[0]);
	...
	public static int[] nextGreaterElement3(
			int[] a) {
		int n = a.length;
	...
	public void towersOfHanoi(int n) {
		...
		for (int i = 1; i <= totalMoves; i++) {
			if (i % 3 == 1) {
				moveDisks1(sourceStack, destStack,
						s, d); // S <-> D
			} else if (i % 3 == 2) {
				moveDisks1(sourceStack, auxStack,
						s, a); // S <-> A
			} else if (i % 3 == 0) {
				moveDisks1(destStack, auxStack, d,
						a); // D <-> A
			}
		}
	}

	private void moveDisks1(
			Stack<Integer> sourceStack,
			Stack<Integer> destStack, char source,
			char dest) {
		if (sourceStack.isEmpty() || (!destStack
				.empty()
				&& sourceStack.peek() > destStack
						.peek())) {
			sourceStack.push(destStack.pop());
			displayMove(dest, source,
					sourceStack.peek());
		} else {
			destStack.push(sourceStack.pop());
			displayMove(source, dest,
					destStack.peek());
		}
	}

	private void displayMove(char s, char d,
			int disk) {
		System.out.println(
				"Disk: " + disk + " from " + s
						+ " destination " + d);
	...
	private void moveDisks2(
			Stack<Integer> sourceStack,
			Stack<Integer> destStack, char source,
			char dest) {
		int topElementInSource = sourceStack
				.isEmpty() ? 0
						: sourceStack.pop();
		int topElementInDest = destStack.isEmpty()
				? 0
				: destStack.pop();
	 ...
	 * T(n) = O(Logn) + T(n-1)
	 */
	public int largestRectangleArea2(
			int[] heights) {
	...
	public int largestRectangleArea2(
		...
		return Utils.max(
				largestRectangleArea2(heights, l,
						m - 1),
	...
	// Using Stack
	public int largestRectangleArea3(
			int[] heights) {
		int n = heights.length;
		if (n == 0) return 0;
		Stack<Integer> stack = new Stack<>();
		int i = 0, width = 0, maxArea = 0,
				topIndex = 0;
	...
	public int celebrityProblem1(int[][] input,
			int n) {
		if (n == 0) return -1;
	...
	// Using Stack
	public int celebrityProblem2(int[][] input,
			int n) {
		if (n == 0) return -1;
	...
	public int celebrityProblem3(int[][] input,
			int n) {
		if (n == 0) return -1;
	...
	public boolean knows(int[][] M, int i,
			int j) {
	...
	public int lengthLongestPath2(String input) {
		...
		for (String s : input.split("\n")) {
			...
			int len = stack.peek() + s.length()
					- lev + 1; // remove "/t", add"/"
			stack.push(len);
			// check if it is file
			if (s.contains(".")) maxLen = Math
					.max(maxLen, len - 1);
		}
