...
public class BTProblems {
	...
	/*
	 * Below categories are similar patterns 
	 * - Print/Path Type 
	 * - Summation Type 
	 * - LCA Type
	 */

	/* All the below categories solved by using only 
	 * two approaches/algorithms
	 * 
	 * Categories:
	 ...
	 * 	2. Root to any node: findPathFromRootToAnyNode() 
	...
	// Path Sum II: find all root-to-leaf paths where each 
	...
	// Find the maximum path sum between two leaves of a binary tree (Similar to Diameter of the tree)
	 ...
	 */
	// Find the max path sum between two leafs of BT
	...
	// Similar to approach2, but using Height Object
	...
	public int diameterOfTree2(TreeNode root,
		...
		int left = diameterOfTree2(root.left, lh);
	...
	//Similar to find the path for the given node
	...
	/* 
	 *  Connect the node at the same level or Populating Next Right ptr
	 *  in Each Node. 
	...
	//Approach2: Level order traversal Iterative Alg 
	//TC: (n); SC: O(1)
	...
	/* Connect the node at the same level or Populating Next 
	 * Right Pointers in Each Node II
	 * Solution for all type of Binary Tree
	 */

	/* Approach1: Level by level traverse.It may looks like
	 * DFS approach, but not. Because this algorithm connects
	 * one level completely and then go to next level. */
	...
	// Find the next pointer: Keep checking next 
	//pointer in the same level
