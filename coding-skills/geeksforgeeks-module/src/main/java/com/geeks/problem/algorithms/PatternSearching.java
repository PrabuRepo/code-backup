...
public class PatternSearching {
	...
	public void rabinKarpAlgorithm(String text, String pattern) {
		...
		for (int i = 0; i <= n - m; i++) {
			...
			if (i < n - m) {
				...
				textHash += (text.charAt(i + m) * Math.pow(prime, m - 1)); // NewHash = X + text(next char) * prime^(m-1);
			}
		}
	}
	...
	// Solve the problem related to Pattern searching
	/******************************** Pattern Searching Utils ****************************/
	// Method to check given pattern is present in the text
	public boolean KMPAlgorithmUtil(String text, String pattern) {
		int m = pattern.length(), n = text.length();
		int[] lps = computeLPSArray(pattern);
	...
	// Method to check given pattern is present in the text
	public boolean rabinKarpAlgorithmUtil(String text, String pattern) {
		int prime = 101;
		long textHash = 0, patternHash = 0;
		int n = text.length(), m = pattern.length();
		// Calculate hash value
