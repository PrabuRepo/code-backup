...
public class StringProblems {
	...
	public boolean oneAway3(String str1, String str2) {
		...
		for (int i = 0; i < Math.min(m, n); i++) {
			...
			if (m == n)
				return str1.substring(i + 1).equals(str2.substring(i + 1)); // Replacement
			else if (m > n)
				return str1.substring(i + 1).equals(str2.substring(i)); // Insert/Removal
			else
				return str1.substring(i).equals(str2.substring(i + 1));// Insert/Removal
		}
	...
	public int findIntegers(int num) {
		// Find the Binary value of num, to calculate the length of binary digits
		StringBuilder sb = new StringBuilder(Integer.toBinaryString(num));
		int len = sb.length();

		// Count number of binary strings without consecutive 1’s: Exactly similar to find the nth fib
	...
	public int count2(int n) { // Here n is no of binary digits.
		int a[] = new int[n];
		int b[] = new int[n];
