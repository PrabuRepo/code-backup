...
public class IntervalPatterns {
	...
	public int minMeetingRooms1(Interval[] intervals) {
		...
		Arrays.sort(endTime);
		int activeMeetings = 0, numMeetingRooms = 0, i = 0,
				j = 0;
		while (i < len && j < len) {
			if (startTime[i] < endTime[j]) {
				activeMeetings++;
				numMeetingRooms = Math.max(numMeetingRooms,
						activeMeetings);
				i++;
			} else {
				activeMeetings--;
	...
	public int minMeetingRooms3(Interval[] intervals) {
		...
		for (Interval i : intervals) {
			timesMap.put(i.start,
					timesMap.getOrDefault(i.start, 0) + 1);
			timesMap.put(i.end,
					timesMap.getOrDefault(i.end, 0) - 1);
		}
		int count = 0, res = 0;
	...
	public static int minPlatformRequired2(int[] arrv,
			int[] dep) {
		TreeMap<Integer, Integer> map = new TreeMap<>();
		for (int i = 0; i < arrv.length; i++) {
			map.put(arrv[i],
					map.getOrDefault(arrv[i], 0) + 1);
			map.put(dep[i],
					map.getOrDefault(dep[i], 0) - 1);
		}
	...
	public List<Interval> insert2(List<Interval> intervals,
		...
		int startingIndex = binarySearch(intervals,
				newInterval);
		i = startingIndex;
	...
	public List<Interval> insert3(List<Interval> intervals,
			Interval newInterval) {
		List<Interval> result = new ArrayList<>();
	...
	public int binarySearch(List<Interval> intervals,
			Interval newInterval) {
		int low = 0;
	...
	public List<Interval> merge(List<Interval> intervals) {
		...
		Collections.sort(intervals,
				Comparator.comparing(i -> i.start));
		...
		for (int i = 1; i < intervals.size(); i++) {
			...
			if (prevInterval.end >= currInterval.start) {
				prevInterval.end = Math.max(
						currInterval.end, prevInterval.end);
			} else {
	...
	public List<Interval> getIntervals1() {
		...
		for (int i = 0; i < list.size(); i++) {
			int s = i;
			while (i < list.size() - 1 && (list.get(i)
					+ 1 == list.get(i + 1)
					|| list.get(i) == list.get(i + 1)))
				i++;

			intervals.add(
					new Interval(list.get(s), list.get(i)));
		}
		return intervals;
	}
	...
	public void addNum2(int val) {
		...
		if (l != null && h != null
				&& tree.get(l).end + 1 == val
				&& h == val + 1) {
			tree.get(l).end = tree.get(h).end;
			tree.remove(h);
		} else
			if (l != null && tree.get(l).end + 1 >= val) {
				tree.get(l).end = Math.max(tree.get(l).end,
						val);
			} else if (h != null && h == val + 1) {
				tree.put(val,
						new Interval(val, tree.get(h).end));
				tree.remove(h);
			} else {
