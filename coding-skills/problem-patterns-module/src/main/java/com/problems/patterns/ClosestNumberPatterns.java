...
public class ClosestNumberPatterns {
	...
	//Approach11: Binary Search
	public List<Integer> findClosestElements11(int[] arr,
			int k, int x) {
		int lo = 0;
		int hi = arr.length - 1;
		while (hi - lo >= k) {
	...
	//Approach13: Binary Search
	//Same like above, only hi varies
	public List<Integer> findClosestElements13(int[] arr,
			int k, int x) {
	...
	//Approach21: Using minheap priority queue
	//Time: O(n*logk)
	//Space: O(k)
	...
	//Approach22: Using maxheap priority queue
	...
	//TODO:Quick Select/Parition: It looks interesting. Try to understand
