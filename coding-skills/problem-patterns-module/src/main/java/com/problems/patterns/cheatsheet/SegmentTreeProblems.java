...
public class SegmentTreeProblems {
	public void sumRange(int[] arr,
			Query[] queries) {
		int n = arr.length;
		int[] segTree = new int[findSegTreeSize(
				n)];
		...
		for (int i = 0; i < queries.length; i++) {
			...
			System.out.println("Sum Range: " + l
					+ " to " + r + " : "
					+ querySumRange(segTree, l, r,
							0, n - 1, 0));
		}
	}

	public void buildSegTree(int[] arr,
			int[] segTree, int l, int h,
			int pos) {
		if (l == h) {
			segTree[pos] = arr[l];
			return;
		...
		int m = l + (h - l) / 2;
		buildSegTree(arr, segTree, l, m,
				2 * pos + 1);
		buildSegTree(arr, segTree, m + 1, h,
				2 * pos + 2);
		segTree[pos] = segTree[2 * pos + 1]
				+ segTree[2 * pos + 2];
	}

	public int querySumRange(int[] segTree,
		...
		return querySumRange(segTree, qLow, qHigh,
				arrLow, m, 2 * pos + 1)
				+ querySumRange(segTree, qLow,
						qHigh, m + 1, arrHigh,
						2 * pos + 2);
	}
	...
	void updateValueUtil(int[] segTree, int l,
			int h, int index, int diff, int pos) {
		if (l > index || h < index) return;
		segTree[pos] = segTree[pos] + diff;
		if (l != h) {
			int mid = l + (h - l) / 2;
			updateValueUtil(segTree, l, mid,
					index, diff, 2 * pos + 1);
			updateValueUtil(segTree, mid + 1, h,
					index, diff, 2 * pos + 2);
		}
	}
	...
	void updateValue(int arr[], int[] segTree,
		...
		arr[i] = new_val;
		updateValueUtil(segTree, 0, n - 1, i,
				diff, 0);
	}
